...
def submit_script_subset(subset, run_number):
    ...
    for i in range(0,len(subset)):
        rawname = 'RAWDataR'+str(run_number)+'S0p'+str(subset[i])
...
def divide_into_subsets(num, subset_length):
    ...
    while start <= (num - 1):
        end = min(start + subset_length, num) 
        subsets.append(list(range(start, end+1)))
        start = end + 1
    return subsets

# makes the scritp excecutable and it run
def send_to_grid(subset, run_number):
    submit_script_subset(subset, run_number)
